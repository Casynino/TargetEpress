...
/**
 * THE ONE PANEL THAT TAKES A PAYMENT.
 *
 * It began as the Record payment block on the cargo page and was already
 * serving two desks from one implementation: Finance banks the money, Support
 * hands the customer's proof up for Finance to agree. Same figures, same fare,
 * same account, same slip — only the action it posts to and the word on the
 * button differ, and the server authorises each of those separately.
 *
 * It lives in a file of its own because the owner asked for the same panel
 * everywhere money is touched: confirming a claim and correcting one have to
 * offer what recording a payment offers, or the desk has to remember which
 * screen can do what. A second implementation of this form is a second place
 * for the rules about a customer's money to drift.
 */
